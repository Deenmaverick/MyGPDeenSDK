        ...
        override fun onBind(position: Int, viewtype: Int) {
            ...
            when(viewtype)
            {
                DATA_AVAILABLE->
                {
                    ...
                    surahCount.text = surahList[position].id.toString()
                    surahName.text = surahList[position].name_simple
                    arbSurah.text = "${if(surahPost<10)0 else ""}${if(surahPost<100)0 else ""}${surahPost}"
                    surahSub.text = "${surahList[position].translated_name.name} • ${surahList[position].verses_count} Ayahs"
