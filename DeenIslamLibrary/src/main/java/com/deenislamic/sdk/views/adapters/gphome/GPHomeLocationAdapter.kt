...
internal class GPHomeLocationAdapter(
    private val stateArray: ArrayList<StateModel>,
    private var selectedState: StateModel?,
    private val customCallback: RamadanCallback? =null
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BaseViewHolder =
        ViewHolder(
            LayoutInflater.from(parent.context)
                .inflate(R.layout.item_deen_gp_location, parent, false)
        )
    ...
    inner class ViewHolder(itemView: View) : BaseViewHolder(itemView) {
        ...
        override fun onBind(position: Int) {
            ...
            itemName.text = if(DeenSDKCore.GetDeenLanguage() == "bn") dataFilter[absoluteAdapterPosition].statebn else dataFilter[absoluteAdapterPosition].stateValue


            if(dataFilter[absoluteAdapterPosition].state == selectedState?.state) {
                itemName.setTextColor(ContextCompat.getColor(itemName.context, R.color.deen_gp_primary))
                ic_right.show()
            }
            else {
            ...
            itemView.setOnClickListener {
                customCallback?.stateSelected(dataFilter[absoluteAdapterPosition])
            }
